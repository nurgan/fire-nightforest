...
int Cubemap::ImageLoad(const char *filename, Image *image) {
	FILE *file;
...
void Cubemap::init()
{
   ...
   Cubemap::Image *image0 = (Cubemap::Image *)malloc(sizeof(Cubemap::Image));
   ...
   for (unsigned int i = 0; i < imgNames.size(); i++) {
      ...
      std::string imgPath = filename + imgNames[i];    

	   if(!Cubemap::ImageLoad(imgPath.c_str(), image0)) {
      ...
      // Load the actual texture data
	   ...
	   glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 
                     0, GL_RGB, image0->sizeX, image0->sizeY,
